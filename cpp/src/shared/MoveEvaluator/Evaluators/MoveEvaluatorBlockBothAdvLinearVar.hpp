...
class MoveEvaluatorBlockBothAdvLinearVar: public IEvaluator {
 public:
  ...
  MoveEvaluatorBlockBothAdvLinearVar(const Weighting &w): w_{w} {
  ...
  double evaluateMineGivenColHeights(const BitBoard &b, const int *colHeights, int maxDropRem) const {
    ...
    for (int c = 1; c < NUM_COLUMNS-1; ++c) numBlockedCols += !isColAccessible[c];
