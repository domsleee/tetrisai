...
class MyRunner:
  def __init__(self, runner_settings: RunnerSettings, run_particle: IRunParticle, performance_logger: PerformanceLogger=None):
    self._logger = log_wrapper.getLogger('runner')
    self._seen = {}
    self._glob_best = 0.0
    self._best_particle = []
  ...
  def run(self):
    ...
    try:
      ...
      self._performance_logger.log_error(self._last_particle_vs)
    #self._show_plot(optimizer)
    return (cost, pos)
  
  def _run_particle_group(self, particle_vs: ParticleGroup):
    self._iteration += 1
    self._last_particle_vs = particle_vs
    ...
    for i, score in enumerate(result_arr):
      ...
      if my_score > self._glob_best:
        ...
        self._logger.debug("new glob best! %0.8f" % self._glob_best)
        self._logger.debug(self._best_particle)
