...
import { IReadNextPiece } from '@/components/Game/IReadNextPiece';
...
describe('ReadNextPiece', async function() {
  ...
  itParam(
    'verify all pieces are correctly matching ${value}',
    pieces,
    async function(piece: string) {
      ...
      const readNextPiece: IReadNextPiece = new ReadNextPiece(
        await setupWithScreenshot(file)
      );
      expect(readNextPiece.getCurrentPieceFromEmulator()).to.equal(
        // @ts-ignore
        Piece[piece]
      );
    }
